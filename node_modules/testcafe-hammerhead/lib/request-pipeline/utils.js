...
async function callResponseEventCallbackForProcessedRequest(ctx, configureResponseEvent) {
    ...
    const responseEvent = new response_event_1.default(configureResponseEvent._requestFilterRule, preparedResponseInfo);
    await ctx.session.callRequestEventCallback(names_1.default.onResponse, configureResponseEvent._requestFilterRule, responseEvent);
}
...
async function callOnRequestEventCallback(ctx, rule, reqInfo) {
    const requestEvent = new request_event_1.default(ctx, rule, reqInfo);
...
async function callOnConfigureResponseEventForNonProcessedRequest(ctx) {
    await ctx.forEachRequestFilterRule(async (rule) => {
        const configureResponseEvent = new configure_response_event_1.default(ctx, rule, configure_response_event_options_1.default.DEFAULT);
