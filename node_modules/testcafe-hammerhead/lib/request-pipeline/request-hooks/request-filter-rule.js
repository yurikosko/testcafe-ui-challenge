...
class RequestFilterRule {
    ...
    async _matchUsingFunctionOptions(requestInfo) {
    ...
    _stringifyObjectOptions() {
        ...
        const msg = stringifiedOptions.filter(option => !!option.value)
            .map(option => {
            const stringifiedOptionValue = lodash_1.isString(option.value) ? `"${option.value}"` : option.value;
            return `${option.name}: ${stringifiedOptionValue}`;
        })
            .join(', ');
        return `{ ${msg} }`;
    }
    async match(requestInfo) {
